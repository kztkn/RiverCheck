...
体感速度改善はrootや共通レンダリング経路を変更せず段階導入する。開催中のLIVE TABLEだけは画面内表示時の `viewport`、グループTOPの開催履歴・プロフィール・ランキング、およびランキングの個人詳細は `intent` を使う。実機でprefetch単独の安定動作を確認した後、主要リンクだけを`NavLink`へ置き換え、`isPending`中は押したリンク自身に小さな読み込み表示を出す。これにより全画面共通hookへ依存せずタップ直後の反応を返す。認証・Cookie再判定のため `reloadDocument` を使っている導線は変更しない。
