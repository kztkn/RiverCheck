...
押下時にはドメインのリバイ計算で画面共通の仮状態を作り、リバイ欄と終了時入力欄へ同時に表示する。操作ごとのcommand IDとaction結果を対応させ、成功時は返却された確定口数へ揃え、失敗時は仮状態だけを破棄する。成功したリバイ・返済・UNDOでは参加者ページと共通rootのloader再検証を省き、確定口数をクライアント内で維持する。ほかの操作や手動更新によるloader再検証が入ればloaderの値へ戻す。新規参加はCookieを発行する従来の通常POSTを維持し、送信中のボタンを直ちに無効化・表示変更する。
リバイ操作の本人照合にはactionで確認済みのgroup IDを渡し、publicCodeからgroupを再検索しない。プロフィール認証と参加者Cookieのハッシュは並列に処理する。
...
- ローカルルールの文言は`domain/rules/local-rules.ts`へ集約し、受付中の参加者画面は開始スタックと同じBB数の返済ルール、72o設定、ボムポット設定を同じ参照用ボトムシートへ描画する。確定結果では表示せず、各ルールの成立や支払いはDBイベント化しない
...
個人ページのloaderは称号コレクション全件を従来どおり1回で取得し、ページ上では装備中を先頭に獲得日の新しい順で最大5件へ絞った横スクロールプレビューだけを描画する。全件表示にはnative dialogを使い、639px以下は最大88dvhのボトムシート、640px以上は中央モーダルとする。獲得済み／未獲得タブは同じloaderデータをクライアント状態で切り替えるため、開閉・タブ切替による追加APIアクセスは行わない。
...
開催の基本情報変更、ゲーム設定変更、開催削除は、主催者認証済みの開催管理actionからserviceを経由して実行する。基本情報は開催名・開催日だけを扱い、初期チップと実卓のSB / BB / BBAは管理画面本文のゲーム設定として分離する。開始スタックBBはserviceが`initial_chips / big_blind_chips`から算出し、repositoryは実卓ブラインド3値と互換用`initial_stack_bb`を同じUPDATEで保存する。repositoryの`UPDATE`と`DELETE`には`group_id`と`status = 'open'`を含め、画面表示後に確定された場合や別グループIDが指定された場合は変更しない。初期チップ変更では`rebuy_chips`も同じSQLで更新し、リバイイベントまたは終了入力が存在する場合は確認なしの更新を拒否する。基本情報変更はgame IDを維持するため参加者用URLを変えず、作成時のWeb Pushは再送しない。
...
finalizedの開催ではgame_resultsを順位順に取得し、参加者用URLと主催者画面の共通コンポーネントで表示する。順位判定とDB保存は整数scoreのまま維持する。新規・更新開催は`games.big_blind_chips`を1BBの正本とし、同値から算出して保存した`games.initial_stack_bb`を既存の結果・統計コードが利用する。blind列導入前の開催ではnullableなblind列をrepositoryで旧`initial_chips / initial_stack_bb`へフォールバックし、過去結果を変えない。共有操作は主催者画面だけに表示する。LINE用テキストはdomainの純粋関数で生成し、計算式は含めない。共有URLはgame UUIDを22文字のBase64URLへ可逆変換した`/r/:resultCode`を使用する。短縮routeは確定済み開催だけを既存の参加者用URLへredirectし、DBへの短縮コード保存や外部短縮サービスは使用しない。コピーはHTTPSまたはlocalhostではClipboard APIを優先し、同一LANのHTTPなど利用できない環境ではtextarea選択とcopy commandへフォールバックする。自動コピーが拒否された場合は選択状態にして手動コピーを案内する。
...
体感速度改善はrootや共通レンダリング経路を変更せず段階導入する。開催中のLIVE TABLEだけは画面内表示時の `viewport`、グループTOPの開催履歴・プロフィール・ランキング、およびランキングの個人詳細は `intent` を使う。実機でprefetch単独の安定動作を確認した後、主要リンクだけを`NavLink`へ置き換え、`isPending`中は押したリンク自身に小さな読み込み表示を出す。これにより全画面共通hookへ依存せずタップ直後の反応を返す。

開催画面loaderは独立して開始できるゲーム取得、主催者Cookie検証、本人プロフィール取得、participant tokenのハッシュ化を同じ`Promise.all`で並列開始し、サーバー側の直列待ちを減らす。ゲームとグループの特定はpublicCode→group→gameの2クエリ直列ではなく、gamesとgroupsを結合した1クエリで取得する。開催管理loaderも主催者検証、ゲーム取得、participant tokenのハッシュ化を並列開始する。グループTOPのgroup取得と開催一覧取得もpublicCodeをキーに並列化し、初期表示までのDB往復を減らす。

開催管理と自分のプレイ画面の往復はGETによる同一開催内ナビゲーションでCookieを書き換えないため、`reloadDocument`を外して通常のSPA遷移を使う。この2導線は表示時点で`viewport` prefetchし、押下時はリンク自身を「開いています」表示へ切り替える。共有用の完全URLはコピー・QR専用とし、アプリ内リンクには相対originのパスを渡してReact Routerのprefetch対象から外れないようにする。グループTOPへ戻るブランド導線も`viewport` prefetchするが、他ルートへの遷移状態に連動させず、ロゴ表示は常に固定する。参加登録・本人変更・ログアウトなどCookieを書き換えるPOSTは従来どおり個別の再読込方針を維持する。

root loaderのプロフィール・主催者認証・所属数・称号通知は同じgroup code内のGET画面遷移では変化しないため再検証せず、既存loader dataを再利用する。POST後と別groupへの移動は標準の再検証判定を維持する。グループTOPはgroupと開催一覧を先に返して画面を描画し、LIVE TABLEのイベント集計だけをPromiseとしてstreamingする。集計失敗時もTOP本体は表示し、ライブ指標だけを省略する。

### 開催画面の更新

参加者・開催管理routeは定期ポーリングやfocus・pageshow・表示復帰を契機にした再検証を行わない。保存・記録・取消後の既存の再検証と、参加者一覧を開いた際の更新を維持する。開催管理上部の「開催進行」はloader済みデータと精算下書きの現在値だけから純粋関数で次アクションを決め、参加者2人未満、終了入力未完了、不正なリバイ回数、リバイ記録差、精算人数不一致、チップ差分、確定可能の順に優先表示する。DB状態や新しいAPIは追加せず、終了入力の進捗も同じデータから算出する。ゲーム設定とローカルルール変更は同じ開催管理actionを`useFetcher`から送信し、成功時はredirectせずaction dataを返す。これによりURL・スクロール位置・開閉中のDOMを維持し、保存状態だけをボタンとトーストへ反映する。参加者のリバイ・返済操作は保存前後とも上部の`RebuyTracker`内に置き、既存の`useFetcher`を再利用する。rootの`TableEventRecorder`は参加者画面だけ右下の固定EVENT入口を表示し、開催管理では見出し内の入口からカスタムDOMイベントで開く。レコーダーのPOST先・権限判定・取消処理は既存のtable-events resourceを共用する。結果入力欄は保存前後とも同じ位置に置き、保存後のリバイ変更は参加者取得時に`submitted_at`以降のイベントの有無を同一DBクエリで確認して再保存を促す。別フォームや二重送信経路は作らない。開催管理の「入力状況を更新」、入力済み参加者の「確定結果を確認する」は共通の`GameRefreshButton`から既存loaderを1回だけ再検証し、遷移・fetcher処理・再検証中は押せない。主催者が確定済みなら同じ参加者routeで結果を表示する。終了時入力はDOMを維持する開閉式にして入力を保持する。LIVE TABLEは前回取得した累計と比較して増加した指標だけを短時間強調する。グループTOPの主開催がopenなら同じloaderで取得済みの参加人数・イベント件数だけをLiveTableMiniへ渡し、追加通信なしで開催中のライブ面へ切り替える。卓イベントの管理画面入口も既存のtable-events resourceとサーバー認可を共有する。
