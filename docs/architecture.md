...
開催画面loaderは独立して開始できるゲーム取得、主催者Cookie検証、本人プロフィール取得、participant tokenのハッシュ化を同じ`Promise.all`で並列開始し、サーバー側の直列待ちを減らす。開催管理と自分のプレイ画面の往復はGETによる同一開催内ナビゲーションでCookieを書き換えないため、`reloadDocument`を外して通常のSPA遷移と`intent` prefetchを使う。参加登録・本人変更・ログアウトなどCookieを書き換えるPOSTは従来どおり個別の再読込方針を維持する。
