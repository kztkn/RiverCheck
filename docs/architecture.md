...
SPA遷移中はrootの`useNavigation`を唯一の共通状態として使い、`idle`以外の間だけ画面上端へ非ブロッキングな進捗バーを表示する。各routeで独自の全画面spinnerを重ねず、`prefers-reduced-motion`では進捗アニメーションを停止する。

prefetchは通信量とのバランスを取り、現在開催のLIVE TABLEは画面内に見えた時点で`viewport`、プロフィール・戦績、開催履歴、ランキングの個別プレイヤー、通常のサイトメニューなどユーザー操作を伴う導線は`intent`を使う。全開催・全プレイヤーを`viewport`で一括先読みしない。主催者認証や本人Cookieの扱いを理由に`reloadDocument`を使用している導線はこの段階では変更せず、SPA化は認証再判定を個別検証してから行う。
