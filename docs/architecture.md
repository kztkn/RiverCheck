...
体感速度改善の第1段階ではrootや共通レンダリング経路を変更せず、既存のReact Router `Link` に限定してprefetchを追加する。開催中のLIVE TABLEだけは画面内表示時の `viewport`、グループTOPの開催履歴・プロフィール・ランキング、およびランキングの個人詳細は `intent` を使う。認証・Cookie再判定のため `reloadDocument` を使っている導線は変更しない。実機確認後に次段階の遷移中UIを個別に検討する。
