...
チップ構成計算は`domain/chip-distribution`の純粋関数へ閉じ込める。正規化済みの各額面をSB候補にし、100BBの完全一致を満たす3〜4額面の整数枚数をbounded searchで探索する。25枚はソフトな目安として最大40枚まで探索し、35枚超だけを強く減点する。SB額面と次の小中額面の枚数、最高額の枚数・金額集中、高額チップ比率、額面間隔を決定的にスコアリングし、最初に見つかった解ではなく最良解を返す。Reactコンポーネントは額面行の追加・削除、結果表示、明示反映だけを担当し、計算前後の額面一覧や配分はDBへ送らない。
...
開催管理では会費・順位別配分を要約付き`details`として通常閉じ、BBレートの要約と公開操作は閉じた状態でも続けて確認できる構造にする。公開ボタンは`GameSettingsFields`直後、チップ検算・結果確定パネルの前に置き、同じform値を`publish-settlement-plan`へ送る。参加者画面は公開済み`bb_rate`を「今日の精算予定」へ常に表示し、0も「なし」として明示する。画面上の黄色は自動算出値・確認・警告、緑またはニュートラルは入力・プリセット・公開操作に使い分ける。
