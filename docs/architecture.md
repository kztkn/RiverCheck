...
開催管理と自分のプレイ画面の往復はGETによる同一開催内ナビゲーションでCookieを書き換えないため、`reloadDocument`を外して通常のSPA遷移を使う。この2導線は表示時点で`viewport` prefetchし、押下時はリンク自身を「開いています」表示へ切り替える。共有用の完全URLはコピー・QR専用とし、アプリ内リンクには相対originのパスを渡してReact Routerのprefetch対象から外れないようにする。グループTOPへ戻るブランド導線も`viewport` prefetchし、押下時はブランド内に読み込み状態を表示する。参加登録・本人変更・ログアウトなどCookieを書き換えるPOSTは従来どおり個別の再読込方針を維持する。
