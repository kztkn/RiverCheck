...
React Router v8 + Cloudflare Workers + PostgreSQLで、開催作成、共有URLからの自己参加、結果入力、チップ総量検算、順位・会費精算、結果確定、ランキング、開催ハイライト、本人プロフィールまでのMVP主要フローを実装しています。開催写真とプレイヤーアイコンは非公開のCloudflare R2へ保存し、Worker経由で配信します。
